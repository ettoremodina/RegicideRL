...
class Game:
    ...
    def _is_valid_combo(self, cards: List[Card]) -> bool:
        ...
        # Separate Animal Companions from other cards
        animal_companions = [c for c in cards if c.value == 1]
        other_cards = [c for c in cards if c.value != 1]
        
        # If we have Animal Companions
        if animal_companions:
            # Case 1: One Animal Companion with one other card
            if len(animal_companions) == 1 and len(other_cards) == 1:
                return True
            # Case 2: Two Animal Companions only
            elif len(animal_companions) == 2 and len(other_cards) == 0:
                return True
            # Case 3: Animal Companion(s) with a valid combo of numbered cards
            elif len(other_cards) > 1:
                # Check if other cards form a valid combo (same value, total <= 10)
                if len(set(card.value for card in other_cards)) == 1:
                    total = sum(card.value for card in other_cards)
                    return total <= 10
                else:
                    return False
            else:
        ...
        # No Animal Companions - check for regular number combos
    ...
    def _apply_suit_powers(self, cards: List[Card], total_attack: int):
        for card in cards:
            ...
            if card.suit == Suit.HEARTS:
                ...
                self.current_enemy.damage_taken += total_attack  # Double damage
    ...
    def _hearts_power(self, value: int):
        ...
        self.tavern_deck.extend(healed_cards)
    ...
    def _defeat_enemy(self):
        # Place enemy in discard pile or on top of tavern deck
        if self.current_enemy.damage_taken == self.current_enemy.health:
            self.tavern_deck.insert(0, self.current_enemy.card)
        else:
